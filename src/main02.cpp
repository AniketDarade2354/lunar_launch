...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        // Rotate continusaly
        shipRotation = shipRotation + 360.0f * deltaTime;    // 45 degress per second
        if(shipRotation >= 360.0f)
        {
            shipRotation = shipRotation - 360.0f;
        }

        // Move in a circle for demo (sin/cos based on time)
        float radius = 0.3f;

        shipPos.x = radius * std::cos(glm::radians(currentFrame * 360.0f));
        shipPos.y = radius * std::sin(glm::radians(currentFrame * 360.0f));
